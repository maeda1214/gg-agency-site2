...
export default async function TalentDetailPage({ params }: TalentPageParams) {
  const { slug } = await params;
  const talent = talents.find((item) => item.slug === slug);
  if (!talent) notFound();

  return <TalentDetail talent={talent} locale="en" basePath="/talent" contactHref="/en/contact" />;
}
